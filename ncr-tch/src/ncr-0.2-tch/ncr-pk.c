		       ...
		       const struct algo_properties_st *sign_hash)
{
	...
	case NCR_ALG_RSA:
		...
		// Right now for Private key, we only support PKCS1_V1_5
		if (key->type == NCR_KEY_TYPE_PRIVATE &&
		    nla_get_u32(nla) != RSA_PKCS1_V1_5) {
			err();
			return -EINVAL;
		}
			  ...
			  unsigned int osg_cnt, size_t * osg_size)
{
	...
	case NCR_ALG_RSA:
		if (ctx->key->type == NCR_KEY_TYPE_PUBLIC)
			cret = rsa_encrypt_key_ex(input, isg_size, output, &osize,
						  NULL, 0, ctx->oaep_hash, ctx->type,
						  &ctx->key->key.pk.rsa);
		else if (ctx->key->type == NCR_KEY_TYPE_PRIVATE)
			cret = rsa_sign_raw(input, isg_size, output, &osize, &ctx->key->key.pk.rsa);
		else {
			err();
			ret = -EINVAL;
			goto fail;
...
pkey_format ncr_pk_get_pemkey_info(char* key, unsigned int size, unsigned int *key_data_offset, unsigned int *key_data_length)
{
	...
	if (tmp == NULL)
		return UNKNOWN;
	memcpy(tmp, key, size);
	tmp[size] = '\0'; //Add terminating \0 to ensure there is no overflow

	if((begin = strstr(tmp, PEM_PKCS8_PRIVKEY_BEGIN)) != NULL &&
	   (end = strstr(tmp, PEM_PKCS8_PRIVKEY_END)) != NULL &&
	    end > begin) {
		*key_data_offset = begin + strlen(PEM_PKCS8_PRIVKEY_BEGIN) - tmp;
		*key_data_length = end - begin - strlen(PEM_PKCS8_PRIVKEY_BEGIN);
		kfree(tmp);
		return PKCS8;
	}

	if((begin = strstr(tmp, PEM_PKCS1_PRIVKEY_BEGIN)) != NULL &&
	   (end = strstr(tmp, PEM_PKCS1_PRIVKEY_END)) != NULL &&
	    end > begin) {
		*key_data_offset = begin + strlen(PEM_PKCS1_PRIVKEY_BEGIN) - tmp;
		*key_data_length = end - begin - strlen(PEM_PKCS1_PRIVKEY_BEGIN);
		kfree(tmp);
		return PKCS1;
	}

	if((begin = strstr(tmp, PEM_PKCS8ENC_PRIVKEY_BEGIN)) != NULL &&
	   (end = strstr(tmp, PEM_PKCS8ENC_PRIVKEY_END)) != NULL &&
	    end > begin) {
		*key_data_offset = begin + strlen(PEM_PKCS8ENC_PRIVKEY_BEGIN) - tmp;
		*key_data_length = end - begin - strlen(PEM_PKCS8ENC_PRIVKEY_BEGIN);
		kfree(tmp);
		return PKCS8ENC;
	}
...
int ncr_pk_pem_decode_to_der(char *pem_data, unsigned int pem_len, char *out, unsigned long *out_len)
{
	...
	ret = base64_decode(pem_data, pem_len, out, out_len);
...
int ncr_pk_privkey_pkcs8_to_pkcs1(char *key, unsigned long *key_size)
{
	...
	ltc_asn1_list pkcs8_privkey_info[4];
	ltc_asn1_list pkcs8_privkey_algo[2];
	char *tmpbuf = NULL;
	unsigned int tmpbuf_len = *key_size;

	if ((err = mp_init(&version)) != CRYPT_OK) {
		err();
		return -EINVAL;
	}

	tmpbuf = kmalloc(tmpbuf_len, GFP_KERNEL);
	if (tmpbuf == NULL) {
		mp_clear(&version);
		err();
		return -EINVAL;
	...
	LTC_SET_ASN1(pkcs8_privkey_algo, 0, LTC_ASN1_OBJECT_IDENTIFIER, tmpoid, sizeof(tmpoid)/sizeof(tmpoid[0]));
	LTC_SET_ASN1(pkcs8_privkey_algo, 1, LTC_ASN1_NULL, NULL, 0);

	//In PKCS8 private key ASN.1 format, there might be an additional attributes field.
	//We don't need it right now. So just ignore it.
	LTC_SET_ASN1(pkcs8_privkey_info, 0, LTC_ASN1_INTEGER, &version, 1UL);
	LTC_SET_ASN1(pkcs8_privkey_info, 1, LTC_ASN1_SEQUENCE, pkcs8_privkey_algo, 2);
	LTC_SET_ASN1(pkcs8_privkey_info, 2, LTC_ASN1_OCTET_STRING, tmpbuf, tmpbuf_len);

	if ((err = der_decode_sequence(key, tmpbuf_len, pkcs8_privkey_info, 3UL)) != CRYPT_OK) {
		mp_clear(&version);
		kfree(tmpbuf);
		err();
		return -EINVAL;
	...
	//Copy back the pkcs1 format key
	*key_size = pkcs8_privkey_info[2].size;
	memcpy(key, tmpbuf, *key_size);
