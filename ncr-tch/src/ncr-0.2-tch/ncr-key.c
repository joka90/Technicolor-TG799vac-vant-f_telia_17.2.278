...
int ncr_key_init(struct ncr_lists *lst)
{
...
#else
	ret =
	    ncr_limits_add_and_check(current_euid().val, task_pid_nr(current),
				     LIMIT_TYPE_KEY);
#endif
	if (ret < 0) {
		err();
	...
	desc = key->desc;
#else
	desc = idr_alloc(&lst->key_idr, key, 0, 0, GFP_KERNEL);
	if (desc < 0)
	{
		mutex_unlock(&lst->key_idr_mutex);
		_ncr_key_item_put(key);
...
#else
	ncr_limits_remove(current_euid().val, task_pid_nr(current), LIMIT_TYPE_KEY);
#endif
	return ret;
}
...
// item_import should be prepared with key_algo, key_flags, etc set, EXCEPT the field key.secret.data and key.secret.size
		...
		const unsigned int size)
{
	...
	pkey_format key_format = UNKNOWN;
	...
	switch (item->type) {
	case NCR_KEY_TYPE_SECRET:
		if (unlikely(size > NCR_CIPHER_MAX_KEY_LEN)) {
			err();
			ret = -EINVAL;
			goto fail;
		}

		memcpy(item->key.secret.data, key, size);
	...
	case NCR_KEY_TYPE_PUBLIC:
		...
		if(item_import->flags & NCR_KEY_FLAG_PEM_FORMAT){
			key_format = ncr_pk_get_pemkey_info(key, size, &offset, &length);
			// Right now we only support PKCS1 and PKCS8
			if ((key_format != PKCS1 && key_format != PKCS8) ||
			    ncr_pk_pem_decode_to_der(key+offset, length, tmp, &tmp_size)) {
				kfree(tmp);
				err();
				ret = -EINVAL;
				goto fail;
			...
			if (key_format == PKCS8 &&
			    ncr_pk_privkey_pkcs8_to_pkcs1(tmp, &tmp_size)) {
				kfree(tmp);
				err();
				ret = -EINVAL;
				goto fail;
