...
static int bcmxtmrt_set_mac_address(struct net_device *dev, void *p);
static int bcmxtmrt_set_mtu(struct net_device *dev, int new_mtu);
...
static const struct net_device_ops bcmXtmRt_netdevops =
   {
      ...
      .ndo_set_mac_address   = bcmxtmrt_set_mac_address,
      .ndo_tx_timeout        = bcmxtmrt_timeout,
      .ndo_get_stats         = bcmxtmrt_query,
      .ndo_change_mtu        = bcmxtmrt_set_mtu
   };
...
/*---------------------------------------------------------------------------
 * int bcmxtmrt_set_mac_address(struct net_device *dev, void *addr)
 * Description:
...
/*---------------------------------------------------------------------------
 * int bcmxtmrt_set_mtu(struct net_device *dev, int new_mtu)
 * Description:
 *    Called to set the mtu. Called due to shell command,
 *    "ifconfig <device_name> mtu x".
 * Returns:
 *    0 if successful or error status
 *---------------------------------------------------------------------------
 */
