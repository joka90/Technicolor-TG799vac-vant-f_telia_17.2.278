		...
		const struct device_prefix *prefix, struct interface *iface, bool add)
{
	...
	if (!add && assignment->enabled) {
		...
		if (prefix->iface) {
			if (iface->ip6table)
				set_ip_source_policy(false, true, IPRULE_PRIORITY_ADDR, &addr.addr,
						128, iface->ip6table, NULL, NULL, true);
		...
		if (prefix->iface && !assignment->enabled) {
			if (iface->ip6table)
				set_ip_source_policy(true, true, IPRULE_PRIORITY_ADDR, &addr.addr,
						128, iface->ip6table, NULL, NULL, true);
