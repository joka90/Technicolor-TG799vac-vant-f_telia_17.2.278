	...
	case 2:	/* TCP port mapping request */
		{
			...
			if(lifetime == 0) {
				...
				if(iport == 0) {
					...
					int proto2;
					char desc[64];
					while(get_redirect_rule_by_index(index, ext_if_name, NULL, 0,
						...
						if(0 == strcmp(iaddr2, senderaddrstr)
						  && 0 == memcmp(desc, "NAT-PMP", 7)) {
							r = _upnp_delete_redir(ext_if_name, eport2, proto2);
					 ...
					 * mappings */
					r = _upnp_delete_redir(ext_if_name, eport, proto);
			   ...
			   || !check_upnp_rule_against_permissions(upnppermlist, num_upnpperm, eport, senderaddr.sin_addr, iport)) {
				...
				if(r==0) {
					if(strcmp(senderaddrstr, iaddr_old)==0
				       && iport==iport_old) {
						...
						if(_upnp_delete_redir(ext_if_name, eport, proto) < 0) {
							syslog(LOG_ERR, "failed to remove port mapping");
							break;
						}
					} else {
				...
				{ /* do the redirection */
					...
					if(upnp_redirect_internal(ext_if_name, NULL, eport, senderaddrstr,
					                          iport, proto, desc,
					                          timestamp) < 0) {
