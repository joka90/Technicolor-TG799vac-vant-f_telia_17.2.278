	...
	while(line < (h->req_buf + h->req_contentoff))
	{
		...
		if(colon)
		{
			...
			else if(strncasecmp(line, "Callback", 8)==0)
			{
				...
				{
					/* failed to find a URL delimited by <>,
					 * use the entire value as URL */
					p = colon + 1;
					while(isspace(*p) && *p != '\r')
						p++;
					n = 0;
...
ProcessHttpQuery_upnphttp(struct upnphttp * h)
{
	static struct {
		...
		{  ROOTDESC_PATH_END , genRootDesc},
		{ WANIPC_PATH_END, genWANIPCn},
		{ WANCFG_PATH_END, genWANCfg},
#ifdef HAS_DUMMY_SERVICE
		{ DUMMY_PATH_END, NULL},
#endif
	...
	p++;	
	// TEST and skip token
	if ( strncmp( get_current_token(), p, 7 ) == 0 )
	{
		p += 7;
	}
	else if ( strncmp( "rootDesc.xml", p, 5 ) == 0 )
	{
		// For rootDesc we dont need the tokens		
	}
	else if(strcmp("POST", HttpCommand) == 0)
	{
		// For POST requests we check in the field itself
	}
	else if ( (strcmp("SUBSCRIBE", HttpCommand) == 0) ||
		  ( strcmp("UNSUBSCRIBE", HttpCommand) == 0 ) )
	{
