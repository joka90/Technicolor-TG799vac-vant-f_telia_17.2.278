...
newSubscriber(const char * extif, const char * eventurl, const char * callback, int callbacklen)
{
	...
	if(strcmp(eventurl, WANCFG_EVENTURL)==0 || 
		strcmp(eventurl, WANCFG_EVENTURL_END)== 0 )
		tmp->service = EWanCFG;
	else if(strcmp(eventurl, WANIPC_EVENTURL)==0 ||
		strcmp(eventurl, WANIPC_EVENTURL_END)==0)
		tmp->service = EWanIPC;
#ifdef ENABLE_L3F_SERVICE
	else if(strcmp(eventurl, L3F_EVENTURL)==0 ||
		strcmp(eventurl, L3F_EVENTURL_END)==0)
		tmp->service = EL3F;
#endif
#ifdef ENABLE_6FC_SERVICE
	else if(strcmp(eventurl, WANIP6FC_EVENTURL)==0 ||
		strcmp(eventurl, WANIP6FC_EVENTURL_END)==0)
		tmp->service = E6FC;
#endif
#ifdef ENABLE_DP_SERVICE
	else if(strcmp(eventurl, DP_EVENTURL)==0 ||
		strcmp(eventurl, DP_EVENTURL_END )==0 )
		tmp->service = EDP;
#endif
	else {
	        syslog(LOG_ERR, "UNKNOWN EVENTURL : %s", eventurl);
	...
	strncpy(tmp->ifname, extif, IFNAMSIZ);
	return tmp;
}
                         ...
                         int timeout)
{
	...
	syslog(LOG_NOTICE, "ext_iface=%s addSubscriber(%s, %.*s, %d)", ext_if_name,
	       eventurl, callbacklen, callback, timeout);
	/*strncpy(uuid, uuidvalue, sizeof(uuid));
	uuid[sizeof(uuid)-1] = '\0';*/
	tmp = newSubscriber(ext_if_name, eventurl, callback, callbacklen);
	if(!tmp)
		return NULL;
	if(timeout)
...
void
upnp_event_var_change_notify(const char *ifname, enum subscriber_service_enum service)
{
	struct subscriber * sub;
	for(sub = subscriberlist.lh_first; sub != NULL; sub = sub->entries.le_next) {
	    syslog(LOG_DEBUG, "subscription sub->ifname=%s and sub->notify=%p", sub->ifname, (void *)sub->notify);
		if(sub->service == service && sub->notify == NULL && !strcmp(sub->ifname, ifname))
		{
		    syslog(LOG_DEBUG, "upnp_event_var_change_notify hit - calling upnp_event_create_notify - sub->ifname=%s", sub->ifname);
...
static void upnp_event_prepare(struct upnp_event_notify * obj)
{
	...
	case EWanCFG:
		xml = getVarsWANCfg(obj->sub->ifname, &l);
		break;
	case EWanIPC:
		xml = getVarsWANIPCn(obj->sub->ifname, &l);
		break;
#ifdef ENABLE_L3F_SERVICE
	case EL3F:
		xml = getVarsL3F(obj->sub->ifname, &l);
		break;
#endif
#ifdef ENABLE_6FC_SERVICE
	case E6FC:
...
void upnpevents_processfds(fd_set *readset, fd_set *writeset)
{
	...
	while(obj != NULL) {
		...
		if(obj->state == EError || obj->state == EFinished) {
			...
			syslog(LOG_DEBUG, "upnpevents_processfds - Remove upnp_event_notify obj - %s", obj->buffer);
	...
	for(sub = subscriberlist.lh_first; sub != NULL; ) {
		...
		if(sub->timeout && curtime > sub->timeout && sub->notify == NULL) {
		    syslog(LOG_DEBUG, "%s %d %s",
		                   "upnpevents_processfds - remove timeouted subscribers - ", sub->service, sub->callback);
			LIST_REMOVE(sub, entries);
			free(sub);
		}
		sub = subnext;
