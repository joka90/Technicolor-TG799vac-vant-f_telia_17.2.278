...
/* 8 */
	...
	{"/manufacturer",manufacturer /*ROOTDEV_MANUFACTURER*/},		/* required */
...
#ifdef ENABLE_EVENTS
static char *
genEventVars(const char *ifname, int * len, const struct serviceDesc * s)
{
	char tmp[16];
	...
	while(v->name) {
		if(v->itype & 0x80) {
			...
			case CONNECTIONSTATUS_MAGICALVALUE:
				...
				str = strcat_str(str, len, &tmplen,
				   upnpallowedvalues[18 + get_wan_connection_status(ifname)]);
				break;
#ifdef ENABLE_6FC_SERVICE
			...
			case PORTMAPPINGNUMBEROFENTRIES_MAGICALVALUE:
				...
				snprintf(tmp, sizeof(tmp), "%d",
				         upnp_get_portmapping_number_of_entries(ifname));
				str = strcat_str(str, len, &tmplen, tmp);
				break;
			case EXTERNALIPADDRESS_MAGICALVALUE:
				...
				if(use_ext_ip_addr)
					...
					if(getifaddr(ifname, ext_ip_addr, INET_ADDRSTRLEN, NULL, NULL) < 0) {
...
char *
getVarsWANIPCn(const char *ifname, int * l)
{
	return genEventVars(ifname, l, &scpdWANIPCn);
}

char *
getVarsWANCfg(const char *ifname, int * l)
{
	return genEventVars(ifname, l, &scpdWANCfg);
}

#ifdef ENABLE_L3F_SERVICE
char *
getVarsL3F(const char *ifname, int * l)
{
	return genEventVars(ifname, l, &scpdL3F);
}
#endif
