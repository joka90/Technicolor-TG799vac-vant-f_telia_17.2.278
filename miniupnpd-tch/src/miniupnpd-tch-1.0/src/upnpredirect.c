               ...
               unsigned int timestamp)
{
	...
	fprintf(fd, "%s:%s:%hu:%s:%hu:%u:%s\n",
	        ((proto==IPPROTO_TCP)?"TCP":"UDP"), extif, eport, iaddr, iport,
	        timestamp, desc);
	fclose(fd);
	...
	while(fgets(line, sizeof(line), fd)) {
		...
		*(desc++) = '\0';
		...
		if(r == -1) {
			...
			lease_file_add(extif, eport, iaddr, iport, proto_atoi(proto),
			               desc, timestamp);
		}
	}
	fclose(fd);
	...
	if(r == 0) {
		...
		if(strcmp(iaddr, iaddr_old)==0 && iport==iport_old) {
            ...
            if(r == 0) {
                ...
                lease_file_add(ext_if_name, eport, iaddr, iport, proto, desc, timestamp);
            }
...
#ifdef ENABLE_LEASEFILE
	lease_file_add(extif, eport, iaddr, iport, proto, desc, timestamp);
#endif
