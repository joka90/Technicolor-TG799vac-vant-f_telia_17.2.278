...
static int parse_extiface_rule(struct extiface_rule_s *extiface_rule, const char * str)
{
    char *token;

    memset(extiface_rule, 0, sizeof(struct extiface_rule_s));

    token = strtok((char *)str, ":");
    if (!token)
        goto error;
    extiface_rule->name = strdup(token);

    token = strtok(NULL, ":");
    if (!token)
        goto error;
    extiface_rule->startip = strdup(token);

    token = strtok(NULL, ":");
    if (!token)
        goto error;
    extiface_rule->endip = strdup(token);
    ...
    else
    {
        if (strcmp(token, "<n/a>") != 0)
            extiface_rule->ifname = strdup(token);
        else
            extiface_rule->ifname = "";
    }

    token = strtok(NULL, ":");
    if (!token)
        goto error;
    extiface_rule->servicetype= strdup(token);

    token = strtok(NULL, ":");
    if (!token)
        goto error;
    extiface_rule->serviceid = strdup(token);

    return 0;

    error:
        free(extiface_rule->name);
        free(extiface_rule->startip);
        free(extiface_rule->endip);
        free(extiface_rule->ifname);
        free(extiface_rule->servicetype);
        free(extiface_rule->serviceid);
    return -1;
...
{
	size_t size = sizeof("uuid:00000000-0000-0000-0000-000000000000");
	int pos = size - 15;
	
	memcpy(uuidvalue_wan, uuidvalue_igd, size);
	switch(uuidvalue_wan[pos]) {
	case '9':
		uuidvalue_wan[pos] = 'a';
		break;
	case 'f':
	...
	memcpy(uuidvalue_wcd, uuidvalue_wan, size);
	switch(uuidvalue_wcd[pos]) {
	case '9':
		uuidvalue_wcd[pos] = 'a';
		break;
	case 'f':
...
init(int argc, char * * argv, struct runtime_vars * v)
{
    ...
    struct extiface_rule_s *extiface_rule = NULL;
	...
	else
	{
		for(i=0; i<(int)num_options; i++)
		{
			...
			{
			case UPNPEXT_IFNAME:
                ext_if_name_default = ary_options[i].value;
                ext_if_name = ext_if_name_default;
				break;
			case UPNPEXT_IP:
				use_ext_ip_addr = ary_options[i].value;
				break;
			case UPNPLISTENING_IP:
			    if (!isdigit(ary_options[i].value[0]))
                {
			        // Interface is specified instead of IP
			        if (addlanaddrsfrominterface(&lan_addrs, ary_options[i].value) != 0)
                    {
                        fprintf(stderr, "can't get addresses from \"%s\" lan interface\n", ary_options[i].value);
                        break;
                    }
                }
			    else
                ...
                break;
            case UPNPEXT_IFACE_RULE:
                extiface_rule = (struct extiface_rule_s *) malloc(sizeof(struct extiface_rule_s));
                if (extiface_rule == NULL)
                {
                    fprintf(stderr, "malloc(sizeof(struct extiface_rule_s)): %m");
                    break;
                }
                if (parse_extiface_rule(extiface_rule, ary_options[i].value) != 0)
                {
                    fprintf(stderr,
                            "can't parse \"%s\" as valid extiface_rule entry\n",
                            ary_options[i].value);
                    free(extiface_rule);
                    break;
                }
                LIST_INSERT_HEAD(&extiface_rules, extiface_rule, list);
				break;
			case UPNPPORT:
			...
			case UPNPMODELURL:
				...
				break;
			case UPNP_WGCLP_SCRIPT:
				strncpy(upnp_wgclp, ary_options[i].value, UPNP_WGCLP_SCRIPT_LEN);
				upnp_wgclp[UPNP_WGCLP_SCRIPT_LEN-1] = '\0';
				break;
	...
	for(i=1; i<argc; i++)
	{
		...
		case 'M':
			if(i+1 < argc)
				strncpy(manufacturer, argv[++i], MANUFACTURER_MAX_LEN);
			else
				fprintf(stderr, "Option -%c takes one argument.\n", argv[i][1]);
			manufacturer[MANUFACTURER_MAX_LEN-1] = '\0';
			break;
		case 'D':
			if(i+1 < argc)
				strncpy(manufacturer_url, argv[++i], MANUFACTURER_URL_MAX_LEN);
			else
				fprintf(stderr, "Option -%c takes one argument.\n", argv[i][1]);
			manufacturer_url[MANUFACTURER_URL_MAX_LEN-1] = '\0';
			break;
		case 'E':
			if(i+1 < argc)
				strncpy(model_name, argv[++i], MODEL_NAME_MAX_LEN);
			else
				fprintf(stderr, "Option -%c takes one argument.\n", argv[i][1]);
			model_name[MODEL_NAME_MAX_LEN-1] = '\0';
			break;
		case 'F':
			if(i+1 < argc)
				strncpy(model_description, argv[++i], MODEL_DESCRIPTION_MAX_LEN);
			else
				fprintf(stderr, "Option -%c takes one argument.\n", argv[i][1]);
			model_description[MODEL_DESCRIPTION_MAX_LEN-1] = '\0';
			break;
		case 'G':
			if(i+1 < argc)
				strncpy(model_url, argv[++i], MODEL_URL_MAX_LEN);
			else
				fprintf(stderr, "Option -%c takes one argument.\n", argv[i][1]);
			model_url[MODEL_URL_MAX_LEN-1] = '\0';
			break;
...
#ifdef ENABLE_LEASEFILE
	/*remove(lease_file);*/
	syslog(LOG_NOTICE, "Reloading rules from lease file");
...
print_usage:
	fprintf(stderr, "Usage:\n\t"
			...
			"\t\t[-M manufacturer] [-D manufacturer_url] [-E model_name] [-F model_description] [-G model_url]\n"
...
static const char * resolve_ext_iface(in_addr_t ip)
{
    struct extiface_rule_s *extiface_rule;
    struct in_addr startip;
    struct in_addr endip;

    for (extiface_rule = extiface_rules.lh_first; extiface_rule != NULL; extiface_rule =
            extiface_rule->list.le_next)
    {
        inet_pton(AF_INET, extiface_rule->startip, &startip);
        inet_pton(AF_INET, extiface_rule->endip, &endip);

        if (memcmp(&ip, &startip, sizeof(ip)) >= 0 && memcmp(&ip, &endip, sizeof(ip)) <= 0)
            return extiface_rule->ifname;
    }

    return ext_if_name_default;
}

static int ext_ifrule_hit(in_addr_t ip)
{
    struct extiface_rule_s *extiface_rule;
    struct in_addr startip;
    struct in_addr endip;
...
main(int argc, char * * argv)
{
	...
	while(!quitting)
	{
		...
		{
			syslog(LOG_NOTICE, "cleaning expired Port Mappings");
		...
		for(e = upnphttphead.lh_first; e != NULL; e = e->entries.le_next)
		{
			if(e->socket >= 0)
			{
				...
				{
					ext_if_name = resolve_ext_iface(e->clientaddr.s_addr);
		...
		if(shttpl >= 0 && FD_ISSET(shttpl, &readset))
		{
			...
			else
			{
				...
				{
					if (extiface_rules.lh_first == NULL || ext_ifrule_hit(clientname.sin_addr.s_addr))
					{
					...
					else
					{
						...
						sockaddr_to_string((struct sockaddr *)&clientname, addr_str, sizeof(addr_str));
						syslog(LOG_ERR, "HTTP connection from %s did not hit an extif_rule, connection terminated", addr_str);
						close(shttp);
					}
				}
			}
